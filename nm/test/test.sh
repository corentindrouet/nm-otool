...
if [ $TESTNM != 0 ]; then
	...
	while IFS='' read -r -d '' filename; do
		...
		if [ $COUNT != 0 ]; then
			echo "32bits test NM : \033[31mFAILED! \033[0m : "$FILE1": see their, mine."
		else
			echo "32bits test NM : \033[32mSUCCESS!\033[0m : "$FILE1
		fi
	done
	[ $? == 1 ] && exit 0;
	#without fat files
	...
	while IFS='' read -r -d '' filename; do
		...
		if [ "$FILE1" != "/bin/bash" -a "$FILE1" != "/bin/sh" -a "$FILE1" != "/bin/sync" ]; then
			...
			COUNT=$(diff their mine | wc -l)
			if [ $COUNT != 0 ]; then
				echo "64bits tests NM: \033[31mFAILED! \033[0m : "$FILE1": see their, mine."
			else
				echo "64bits tests NM: \033[32mSUCCESS!\033[0m :"$FILE1
			fi
		fi
	done
	[ $? == 1 ] && exit 0;
	...
	COUNT=$(diff their mine | wc -l)
	if [ $COUNT != 0 ]; then
		echo "Fat tests NM: \033[31mFAILED! \033[0m : /bin/bash: see their, mine."
	else
		echo "Fat tests NM: \033[32mSUCCESS!\033[0m : /bin/bash"
	fi
	[ $? == 1 ] && exit 0;
	nm /bin/sh > their ; ../ft_nm /bin/sh > mine
	COUNT=$(diff their mine | wc -l)
	if [ $COUNT != 0 ]; then
		echo "Fat tests NM: \033[31mFAILED! \033[0m : /bin/sh: see their, mine."
	else
		echo "Fat tests NM: \033[32mSUCCESS!\033[0m : /bin/sh"
	fi
	[ $? == 1 ] && exit 0;
	nm /bin/sync > their ; ../ft_nm /bin/sync > mine
	COUNT=$(diff their mine | wc -l)
	if [ $COUNT != 0 ]; then
		echo "Fat tests: \033[31mFAILED! \033[0m : /bin/sync: see their, mine."
	else
		echo "Fat tests: \033[32mSUCCESS!\033[0m : /bin/sync"
	fi
	[ $? == 1 ] && exit 0;
	...
	while IFS='' read -r -d '' filename; do
		...
		COUNT=$(diff their mine | wc -l)
		if [ $COUNT != 0 ]; then
			echo "Archive tests NM: \033[31mFAILED! \033[0m : "$FILE1": see their, mine."
		else
			echo "Archive tests NM: \033[32mSUCCESS!\033[0m :"$FILE1
		fi
	done
	...
	while IFS='' read -r -d '' filename; do
		...
		if [ "$FILE1" != "" -a "$FILE1" != "/usr/lib/libnetsnmp.5.2.1.dylib" -a "$FILE1" != "/usr/lib/libsqlite3.dylib" ]; then
			...
			if [ $COUNT != 0 ]; then
				echo "/usr/lib tests NM: \033[31mFAILED! \033[0m : "$FILE1": see their, mine."
			else
				echo "/usr/lib tests NM: \033[32mSUCCESS!\033[0m :"$FILE1
			fi
		fi
	done
	...
	while IFS='' read -r -d '' filename; do
		...
		if [ "$FILE1" != "/usr/lib/libnetsnmp.5.2.1.dylib" ]; then
			...
			if [ $COUNT != 0 ]; then
				echo "/sbin tests NM: \033[31mFAILED! \033[0m : "$FILE1": see their, mine."
			else
				echo "/sbin tests NM: \033[32mSUCCESS!\033[0m :"$FILE1
			fi
	...
	while IFS='' read -r -d '' filename; do
		...
		COUNT=$(cat their mine | sort | uniq -u | wc -l)
		if [ $COUNT != 0 ]; then
			echo "/usr/bin tests NM: \033[31mFAILED! \033[0m : "$FILE1": see their, mine."
		else
			echo "/usr/bin tests NM: \033[32mSUCCESS!\033[0m :"$FILE1
		fi
	done
	...
	while IFS='' read -r -d '' filename; do
		...
		COUNT=$(cat their mine | sort | uniq -u | wc -l)
		if [ $COUNT != 0 ]; then
			echo "/usr/sbin tests NM: \033[31mFAILED! \033[0m : "$FILE1": see their, mine."
		else
			echo "/usr/bin tests NM: \033[32mSUCCESS!\033[0m :"$FILE1
		fi
	done
...
if [ $TESTOTOOL != 0 ]; then
	...
	while IFS='' read -r -d '' filename; do
		...
		COUNT=$(diff their mine | wc -l)
		if [ $COUNT != 0 ]; then
			echo "32bits test OTOOL : \033[31mFAILED! \033[0m : "$FILE1": see their, mine."
			exit 1
		else
			echo "32bits test OTOOL : \033[32mSUCCESS!\033[0m : "$FILE1
		fi
	done
	[ $? == 1 ] && exit 0;
	#without fat files
	...
	while IFS='' read -r -d '' filename; do
		...
		if [ "$FILE1" != "/bin/bash" -a "$FILE1" != "/bin/sh" -a "$FILE1" != "/bin/sync" ]; then
			...
			COUNT=$(diff their mine | wc -l)
			if [ $COUNT != 0 ]; then
				echo "Standard tests OTOOL: \033[31mFAILED! \033[0m : "$FILE1": see their, mine."
				exit 1
			else
				echo "Standard tests OTOOL: \033[32mSUCCESS!\033[0m :"$FILE1
			fi
		fi
	done
	[ $? == 1 ] && exit 0;
	...
	COUNT=$(diff their mine | wc -l)
	if [ $COUNT != 0 ]; then
		echo "Fat tests OTOOL: \033[31mFAILED! \033[0m : /bin/bash: see their, mine."
		exit 1
	else
		echo "Fat tests OTOOL: \033[32mSUCCESS!\033[0m : /bin/bash"
	fi
	[ $? == 1 ] && exit 0;
	otool -t /bin/sh > their ; ../ft_otool /bin/sh > mine
	COUNT=$(diff their mine | wc -l)
	if [ $COUNT != 0 ]; then
		echo "Fat tests OTOOL: \033[31mFAILED! \033[0m : /bin/sh: see their, mine."
		exit 1
	else
		echo "Fat tests OTOOL: \033[32mSUCCESS!\033[0m : /bin/sh"
	fi
	[ $? == 1 ] && exit 0;
	otool -t /bin/sync > their ; ../ft_otool /bin/sync > mine
	COUNT=$(diff their mine | wc -l)
	if [ $COUNT != 0 ]; then
		echo "Fat tests OTOOL: \033[31mFAILED! \033[0m : /bin/sync: see their, mine."
		exit 1
	else
		echo "Fat tests OTOOL: \033[32mSUCCESS!\033[0m : /bin/sync"
	fi
	[ $? == 1 ] && exit 0;
	...
	while IFS='' read -r -d '' filename; do
		...
		COUNT=$(diff their mine | wc -l)
		if [ $COUNT != 0 ]; then
			echo "Archive tests OTOOL: \033[31mFAILED! \033[0m : "$FILE1": see their, mine."
			exit 1
		else
			echo "Archive tests OTOOL: \033[32mSUCCESS!\033[0m :"$FILE1
		fi
	done
	...
	while IFS='' read -r -d '' filename; do
		...
		if [ "$FILE1" != "/usr/lib/libkmodc++.a" -a "$FILE1" != "/usr/lib/libnetsnmp.5.2.1.dylib" -a "$FILE1" != "/usr/lib/libsqlite3.dylib" ]; then
			...
			if [ $COUNT != 0 ]; then
				...
				echo "/usr/lib tests OTOOL: \033[32mSUCCESS!\033[0m :"$FILE1
			fi
		fi
	done
	...
	while IFS='' read -r -d '' filename; do
		...
		if [ "$FILE1" != "/usr/lib/libnetsnmp.5.2.1.dylib" ]; then
			...
			COUNTEM=$(cat mine_err | grep "Permission denied" | wc -l)
			if [ $COUNT != 0 ]; then
				echo "/sbin tests OTOOL: \033[31mFAILED! \033[0m : "$FILE1": see their, mine."
				exit 1
			else
				echo "/sbin tests OTOOL: \033[32mSUCCESS!\033[0m :"$FILE1
			fi
			if [ $COUNTET != $COUNTEM ]; then
				exit 1
			fi
	...
	while IFS='' read -r -d '' filename; do
		...
		COUNT=$(cat their mine | sort | uniq -u | wc -l)
		if [ $COUNT != 0 ]; then
			echo "/usr/bin tests OTOOL: \033[31mFAILED! \033[0m : "$FILE1": see their, mine."
			exit 1
		else
			echo "/usr/bin tests OTOOL: \033[32mSUCCESS!\033[0m :"$FILE1
		fi
	done
	...
	while IFS='' read -r -d '' filename; do
		...
		COUNT=$(cat their mine | sort | uniq -u | wc -l)
		if [ $COUNT != 0 ]; then
			echo "/usr/sbin tests OTOOL: \033[31mFAILED! \033[0m : "$FILE1": see their, mine."
			exit 1
		else
			echo "/usr/bin tests OTOOL: \033[32mSUCCESS!\033[0m :"$FILE1
		fi
